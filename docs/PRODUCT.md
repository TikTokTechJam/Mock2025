...
| Runtime model artifact resolver | `apps/api/src/privastream_api/model_artifacts.py` and `models/manifests/` | Registers ML handoff metadata for files or model packs, resolves logical model IDs into a versioned local cache, verifies SHA-256, extracts bounded archives, and supplies runtime metadata to adapters. No production artifact is committed. | Implemented | Unverified |
